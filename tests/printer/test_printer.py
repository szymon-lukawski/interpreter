...
def test_print_add_interpretable_as_int_str_and_int_minus_float():
    ...
    printer = Printer()
    expected = "AddExpr([StrLiteral('1'), IntLiteral(2), FloatLiteral(3.4)], ['+', '-'])"
    result = printer.print(ast)
    assert expected == result
...
def test_print_add_noninterpretable_as_int_str_and_int_minus_float():
    ...
    printer = Printer()
    expected = "AddExpr([StrLiteral('abc'), IntLiteral(2), FloatLiteral(3.4)], ['+', '-'])"
    result = printer.print(ast)
    assert expected == result
...
def test_print_zero_or_zero_as_float_or_zero_as_int_str_or_zero_as_float_str_or_one():
    """0|0.0|'0'|'0.0'|1"""
    ast = OrExpr([IntLiteral(0), FloatLiteral(0.0), StrLiteral('0'), StrLiteral('0.0'), IntLiteral(1)])
    printer = Printer()
...
def test_print_and_all_types_interpetable_as_true():
    """1&1.2&'1'&'1.2'"""
    ast = AndExpr([IntLiteral(1), FloatLiteral(1.2), StrLiteral('1'), StrLiteral('1.2')])
    printer = Printer()
...
def test_and_or_and():
    """0&1|2&3"""
    ast = OrExpr([AndExpr([IntLiteral(0), IntLiteral(1)]), AndExpr([IntLiteral(0), IntLiteral(1)])])
    printer = Printer()
...
def test_or_and_or():
    """0|1&2|3"""
    ast = OrExpr([IntLiteral(0), AndExpr([IntLiteral(1), IntLiteral(2)]), IntLiteral(3)])
    printer = Printer()
...
def test_assignment_more_complicated_expr():
    """a=(1+2*3)/4"""
    ast = AssignmentStatement(ObjectAccess(['a']), MultiExpr([AddExpr([IntLiteral(1), MultiExpr([IntLiteral(2), IntLiteral(3)], ['*'])], ['+']), IntLiteral(4)], ['/']))
